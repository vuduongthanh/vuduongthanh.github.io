...
<p style="text-align: center"><h1 ><b> OFF TOKEN WINDOWS</b></h1><br> </p>
...
<form method="post">
   ...
   <br>
    <input type="submit" id="check"  name="submit"  value="CHECK" >&nbsp;
  ...
  function string_between_two_string($str, $starting_word, $ending_word)
{
    ...
    return substr($str, $subtring_start, $size);
     ...
     $str = base64_decode($allstring);
              ...
              // $FD=$_POST["allstring"];
             // echo    $FD=$_POST[$str];
             $tokenstart=strpos($str,'"token":"',0);
             $MDstart=strpos($str,'"X-Apple-I-MD = ',0);
             $MDMstart=strpos($str,'"X-Apple-I-MD-M = ',0);
             $dsidstart = strpos($str,'"dsid":"',0);

				echo $dsid = string_between_two_string($str,'"dsid":"','","token"');
				//echo $basic1=substr($str,$tokenstart+9,$length+204);
				echo $basic1=string_between_two_string($str,'"token":"','"}@@{"') ;
				//echo $mdm=substr($str,$MDMstart+18,$length+80);
				$mdm=string_between_two_string($str,'"X-Apple-I-MD-M =','"}') ;
				echo $mdm =trim($mdm,'" ') ;
				//echo $md=substr($str,$MDstart+16,$length+40);
